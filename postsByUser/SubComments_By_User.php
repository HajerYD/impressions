...
<html>
	<head>
		<title>SubComments By User</title>
	</head>
	<body>
        ...
        <h2>SubComments By <?=$_GET['username']?></h2>
        <!--div for the list of subcomments by the user-->
        <div>
        <?php
                ...
                $query1 = "SELECT * FROM UserMakesSubPost WHERE username='" . $_GET['username'] . "'";
                $result1 = mysql_query($query1, $connection) 
                    or die("Query 1 failed: " . mysql_error() . "<br />");
                while($row1=mysql_fetch_array($result1)) {  
                	//get all of the information about each comment
               		$query2 = "SELECT * FROM SubComments WHERE subPostID='" . $row1['subPostID'] . "'";
               		$result2 = mysql_query($query2, $connection)
               			or die("Query 2 failed: " . mysql_error() . "<br />"); 
               		$row2 = mysql_fetch_array($result2);
               		
               		//get the course information where the comment was made
		           	$query3 = "SELECT * FROM Courses WHERE courseID IN (SELECT courseID FROM CommentInCourseInSemester WHERE postID IN (SELECT mainPostID FROM SubCommentOfComment WHERE subPostID='" . $row1['subPostID'] . "'))";
		          	$result3 = mysql_query($query3, $connection)
		           	...
		           	while($row3 = mysql_fetch_array($result3)) {	        		
				       	...
				       	echo "<textarea style='width:500px;height:100px;max-width:500px;max-height:100px;' cols='50' readonly='readonly'>$row2[subCommentPost]</textarea><br />";
